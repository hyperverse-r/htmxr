...
knitr::include_graphics("man/figures/intro_htmxr.png")
```
...
```{r, echo=FALSE, out.width="50%"}
knitr::include_graphics("man/figures/intro_htmxr_hello.png")
```
...
- **CSS-agnostic** — `{htmxr}` doesn't depend on any CSS framework. Use Bootstrap, Tailwind, or nothing at all.
- **htmltools re-exported** — `tags`, `div`, `p`, etc. are available directly from `{htmxr}`.
